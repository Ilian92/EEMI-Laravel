<x-layout>
    ...
    <section class="relative h-64 md:h-80 bg-gradient-to-br from-blue-50 to-indigo-100 overflow-hidden">
        <img src="{{ $user->banner_image_url }}" alt="{{ __('Bannière de :name', ['name' => $user->name]) }}"
                    ...
                    <div class="flex-1">
                            ...
                            @auth
                                @if(Auth::id() !== $user->id && $user->is_creator)
                                    <div class="mt-4 md:mt-0">
                                        @if($isSubscribed)
                                            <form method="POST" action="{{ route('user-profile.unsubscribe', $user) }}"
                                                class="inline">
                                                @csrf
                                                <button type="submit"
                                        ...
                                        @else
                                            <form method="POST" action="{{ route('user-profile.subscribe', $user) }}"
                                                class="inline">
                                                @csrf
                                                <button type="submit"
                        ...
                        <div class="flex space-x-6 mt-4">
                            ...
                            @if($user->creator_since)
                                <div class="text-center">
                                    <div class="text-2xl font-bold" style="color: #00aff0;">
                                        {{ max(1, ceil($user->creator_since->diffInMonths())) }}
                                    </div>
    ...
    <section class="py-12 bg-gray-50">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            @if($user->is_creator)
                <div class="mb-8">
                    <h2 class="text-2xl font-bold mb-4">Posts récents</h2>

                    @forelse ($posts as $post)
                        <article class="mb-6 bg-white rounded-lg shadow p-4">
                            <p class="text-gray-800 mb-2">{!! nl2br(e($post->content)) !!}</p>
                            ...
                            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                                <div class="text-center py-12">
                                    ...
                                    <p class="text-gray-600">
                                        {{ __('Ce créateur travaille sur du contenu exclusif. Abonnez-vous pour être notifié !') }}
                                    </p>
                                </div>
                            </div>
                        </section>
